...
class CacheWrapperResourceLoaderTest extends SecurityTestCase
{
    ...
    public function testLoadResourceFromCache(): void
    {
        ...
        $this->assertInstanceOf(ResourceInterface::class, $wrapper->loadResource("Foo"));
        $loaded = $this->reflection_getPropertyValue($wrapper, $reflection, "loaded");
        $this->assertInstanceOf(ResourceInterface::class, $loaded["Foo"]);
    }
    ...
    public function testRegister(): void
    {
        ...
        $cache = $this->getMockBuilder(CacheInterface::class)->setMethods($this->getPSR16MethodsInterface())->getMock();
        $cache->expects($this->once())->method("get")->with(CacheWrapperResourceLoader::CACHE_RESOURCE_PREFIX."_Names")->will($this->returnValue(null));
        $cache->expects($this->once())->method("set")->with(CacheWrapperResourceLoader::CACHE_RESOURCE_PREFIX."_Names", $cacheValue)->will($this->returnValue(true));
    ...
    public function testRegisterFromCache(): void
    {
        ...
        $cache = $this->getMockBuilder(CacheInterface::class)->setMethods($this->getPSR16MethodsInterface())->getMock();
        $cache->expects($this->once())->method("get")->with(CacheWrapperResourceLoader::CACHE_RESOURCE_PREFIX."_Names")->will($this->returnValue($cacheValue));
        $wrapper = new CacheWrapperResourceLoader($loader, $cache);
    ...
    public function testClear(): void
    {
        ...
        $keys[] = CacheWrapperResourceLoader::CACHE_RESOURCE_PREFIX."_Names";
        $cache = $this->getMockBuilder(CacheInterface::class)->setMethods($this->getPSR16MethodsInterface())->getMock();
